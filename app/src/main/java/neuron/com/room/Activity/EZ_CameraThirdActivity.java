...
import neuron.com.comneuron.RootActivity;

/**
 * Created by ljh on 2017/2/15.  closeActivity（）方法中是最后的逻辑处理方法
 */
public class EZ_CameraThirdActivity extends RootActivity implements View.OnClickListener{
    ...
    Handler defiveFindHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            if (msg.what == 0) {
                EZConstants.EZWifiConfigStatus status = (EZConstants.EZWifiConfigStatus) msg.obj;
                if (status.getStatus() == EZConstants.EZWifiConfigStatus.DEVICE_WIFI_CONNECTING.getStatus()) {

                } else if (status.getStatus() == EZConstants.EZWifiConfigStatus.DEVICE_WIFI_CONNECTED.getStatus()) {
                    if (isWifiConnected) {
                ...
                } else if (status.getStatus() == EZConstants.EZWifiConfigStatus.DEVICE_PLATFORM_REGISTED.getStatus()) {
...
//    private UnbindDeviceTriggerHelper mTriggerHelper;
    ...
    private int probeDeviceInfo(String deviceSerial) {
        try {
            mEZProbeDeviceInfo = OgeApplication.getOpenSDK().probeDeviceInfo(serialNo);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.ezcamerathird);
        // 唤醒，常亮
...
//            mDeviceInfoEx = DeviceManager.getInstance().getDeviceInfoExById(serialNo);
    ...
    private void init() {
        ...
        serialNo = getIntent().getStringExtra("cameraSerial");
        mVerifyCode = getIntent().getStringExtra("cameraVerification");
        wifiPassword = getIntent().getStringExtra("wifipwd");
        //deviceType = getIntent().getStringExtra();
...
//        mTriggerHelper.onActivityResult(requestCode, resultCode, intent);
    ...
    private void showStatus(int status) {
        ...
        tip.setTextSize(Utils.px2dip(this, getResources().getDimension(R.dimen.text_size_14)));
        tip2.setTextSize(Utils.px2dip(this, getResources().getDimension(R.dimen.text_size_14)));
        tip3.setTextSize(Utils.px2dip(this, getResources().getDimension(R.dimen.text_size_14)));
        tip.setTextColor(getResources().getColor(R.color.auto_tip_text));
        tip2.setTextColor(getResources().getColor(R.color.auto_tip_text));
        tip3.setTextColor(getResources().getColor(R.color.auto_tip_text));
        tip3.setVisibility(View.VISIBLE);
        // 连接wifi
        if (STATUS_WIFI_CONNETCTING == status) {
            tip.setTextSize(Utils.px2dip(this, getResources().getDimension(R.dimen.text_size_16)));
            tip.setTextColor(getResources().getColor(R.color.text_black_333333));
            ...
            tip2.setTextSize(Utils.px2dip(this, getResources().getDimension(R.dimen.text_size_16)));
            tip2.setTextColor(getResources().getColor(R.color.text_black_333333));
            ...
            successIcon3.setVisibility(View.INVISIBLE);
            tip3.setTextSize(Utils.px2dip(this, getResources().getDimension(R.dimen.text_size_16)));
            tip3.setTextColor(getResources().getColor(R.color.text_black_333333));
            tip.setText(R.string.auto_wifi_tip_connecting_wifi_ok);
...
//                            LocalInfo.getInstance().setWifiPassword(mac, wifiPassword);
        ...
        OgeApplication.getOpenSDK().stopConfigWiFi();
        OgeApplication.getOpenSDK().startConfigWifi(EZ_CameraThirdActivity.this, serialNo, wifiSSID, wifiPassword, mEZStartConfigWifiCallback);
    }
...
//        WebUtils.openYsCloudIntro(this);
    ...
    private void enableCloudStoryed() {
        ...
        if (!ConnectionDetector.isNetworkAvailable(this)) {
            Toast.makeText(EZ_CameraThirdActivity.this,R.string.save_encrypt_password_fail_network_exception,Toast.LENGTH_LONG).show();
            return;
        }
...
//                                LocalInfo.getInstance().setWifiPassword(mac, wifiPassword);
            ...
            case STATUS_ADD_CAMERA_SUCCESS:
                ...
                if (bX1orX2) {
                    imgAnimation.setImageResource(R.mipmap.success_img);
                } else {
                    imgAnimation.setImageResource(R.mipmap.equipment_complete);
                }
                // DeviceModel.A1
    ...
    private void addCameraFailed(int errorStep, int errorCode) {
        ...
        switch (errorStep) {
            case ERROR_WIFI_CONNECT:
                ...
                imgAnimation.setImageResource(R.mipmap.camera_disconnect);
...
//        }
    ...
    private void addQueryCameraAddVerifyCode() {
        // 本地网络检测
        if (!ConnectionDetector.isNetworkAvailable(this)) {
            Toast.makeText(EZ_CameraThirdActivity.this,R.string.add_camera_fail_network_exception,Toast.LENGTH_LONG).show();
            return;
        }
        new Thread() {
            public void run() {
                ...
                while (count > 0) {
                    ...
                    try {
                        OgeApplication.getOpenSDK().addDevice(serialNo, mVerifyCode);
